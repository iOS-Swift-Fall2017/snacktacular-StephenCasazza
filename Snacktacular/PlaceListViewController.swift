...
class PlaceListViewController: UIViewController {
    ...
    func loadData() {
        db.collection("places").getDocuments { (querySnapshot, error) in
            ...
            for documents in querySnapshot!.documents {
                let placeDocumentID = documents.documentID
                ...
                let latitude = docData["latitude"] as? CLLocationDegrees ?? 0
                let longitude = docData["longitude"] as? CLLocationDegrees ?? 0
    ...
    func saveData(index: Int) {
        ...
        //If we have saved a record we will have an ID
        if places[index].placeDocumentID != "" {
            ...
            ref = db.collection("places").addDocument(data: dataToSave) { (error) in
                if let error = error {
                    ...
                    print("Doc added with string ID \(ref?.documentID)")
...
extension PlaceListViewController: FUIAuthDelegate {
    ...
    func authUI(_ authUI: FUIAuth, didSignInWith user: User?, error: Error?) {
        if let user = user {
            print("Successfully logged in with \(user.email!)")
        }
    }
